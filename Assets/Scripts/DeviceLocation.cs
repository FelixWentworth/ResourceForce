...
public class DeviceLocation : MonoBehaviour {
    ...
    public void OKSelected()
    {
        if (_locationIndex == 0)
        {
            ...
            StartCoroutine(WarningBox.ShowWarning(Localization.Get("WARNING_TEXT_LOCATION")));

        } 
        if (!IsAnyLanguageSelected())
        {
            // notify the player they need to select a language
            StartCoroutine(WarningBox.ShowWarning(Localization.Get("WARNING_TEXT_LANGUAGE")));
        }
        else
        {
