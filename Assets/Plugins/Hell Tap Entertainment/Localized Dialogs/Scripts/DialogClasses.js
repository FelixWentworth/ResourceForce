...
// Transitions
enum DUI_TransitionEffects{ None, PushLeft, PushRight, PushDown, PushUp, InAndOutFromLeft, InAndOutFromRight, InAndOutFromTop, InAndOutFromBottom, Popup, Eyelids, BarnDoors, Zoom, ZoomHorizontal, ZoomVertical, Spin, SpinPopup, SpinZoom }				

// Overrides
enum DIALOG_OVERRIDE_YESNO{UseDefault,Yes,No}				

enum DIALOG_OVERRIDE_SCROLLING{UseDefault,Off,AutomaticScrolling,ManualScrollingWithVerticalBar}

enum DIALOG_OVERRIDE_TRANSITION{ UseDefault, None, PushLeft, PushRight, PushDown, PushUp, InAndOutFromLeft, InAndOutFromRight, InAndOutFromTop, InAndOutFromBottom, Popup, Eyelids, BarnDoors, Zoom, ZoomHorizontal, ZoomVertical, Spin, SpinPopup, SpinZoom }
...
class DS_Screen {
	...
	// ** NEW in 4.5 -> Set scrolling and typewriter options per screen
	var typeWriterOptions : DIALOG_OVERRIDE_YESNO = DIALOG_OVERRIDE_YESNO.UseDefault;		// Override typewriter options per screen
	var scrollingOptions : DIALOG_OVERRIDE_SCROLLING = DIALOG_OVERRIDE_SCROLLING.UseDefault;	// Override scrolling options
	...
	// ** NEW in 3.3 -> Titles (Extra options added in 4.5!)
	...
	var titleSize : Vector2 = Vector2(960,640);									// Size of the title
	var titleFontSize : int = 0;												// Size of font - 0 means use default.
	var titleAllignment : TextAnchor = TextAnchor.UpperLeft;					// allignment of title	
	var titleColor : Color = Color.white;										// Text color of the title.
	var overrideTitleFont : Font = null;										// Override the font of this title
	var subtitleColor : Color = Color.white;									// Text color of the subtitle.
	var subtitleSize : Vector2 = Vector2(960,640);								// Size of the subtitle
	var subtitleFontSize : int = 0;												// Size of font - 0 means use default.
	var subtitleAllignment : TextAnchor = TextAnchor.UpperLeft;					// allignment of subtitle				
	var overrideSubtitleFont : Font = null;										// Override the font of this subtitle
	...
	var IG_buttonAllignment : TextAnchor = TextAnchor.MiddleCenter;					// How should the button text/image be alligned
	...
	class IconGridButtons{
		...
		// Icon
		var buttonIcon : Texture2D = null;											// Custom Button 1 Icon
		var animatedButtonIcon : Vector2 = Vector2(-1,-1);							// The current Group, then animation ID to use (-1 means dont animate) ***
		#if UNITY_EDITOR
			@System.NonSerialized
			var editorButtonAnim : DialogCastActor = new DialogCastActor();			// Caches the animation temporarily in the editor.
		#endif
...
class DS_Navigation {
	...
	var screenTransitionIn : DIALOG_OVERRIDE_TRANSITION = DIALOG_OVERRIDE_TRANSITION.UseDefault;	// NEW in 4.6 - screen transitions!
	var screenTransitionOut : DIALOG_OVERRIDE_TRANSITION = DIALOG_OVERRIDE_TRANSITION.UseDefault;	// NEW in 4.6 - screen transitions!
	var endDialogAfterThis : boolean = false;
	var destroyAtEnd : boolean = false;
...
class DS_Actions{
	...
	var activateTheseObjectsAtStartDirectly : GameObject[] = new GameObject[0];		// ** Find And Activate these Objects when the screen has started -> from v2.9
	var activateTheseObjectsAtEnd : GameObject[] = new GameObject[0];				// Activate these Objects when the screen has ended
	var activateTheseObjectsAtEndByName : String[] = new String[0];					// ** Find And Activate these Objects when the screen has ended -> from v2.9
	
	// Object De-Activation
	var deactivateTheseObjectsAtStart : String[] = new String[0];					// Find And Activate these Objects when the screen has started -> from v2.9
	var deactivateTheseObjectsAtStartDirectly : GameObject[] = new GameObject[0];	// ** Find And Activate these Objects when the screen has started -> from v2.9
	var deactivateTheseObjectsAtEnd : GameObject[] = new GameObject[0];				// Activate these Objects when the screen has ended -> from v2.9
	var deactivateTheseObjectsAtEndByName : String[] = new String[0];				// ** Find And Activate these Objects when the screen has ended -> from v2.9

	// Debug Log
	var showMessageInConsoleAtStart : String[] = new String[0];						// writes a string (with tokens) in the console at start.
	var showMessageInConsoleAtEnd : String[] = new String[0];						// writes a string (with tokens) in the console at end.

	// Destroy GameObjects
	var findAndDestroyTheseObjectsAtStart : String[] = new String[0];				// Find And destroy these game objects when the screen has started -> from v2.9
	var findAndDestroyTheseObjectsAtEnd : String[] = new String[0];					// Find And destroy these game objects when the screen has ended
	var destroyTheseObjectsAtStart : GameObject[] = new GameObject[0];				// Destroy these game objects when the screen has started -> from v2.9
	var destroyTheseObjectsAtEnd : GameObject[] = new GameObject[0];				// Destroy these game objects when the screen has started -> from v2.9
	...
	// RT Voice Actions
	var rtVoice : DSActionsFor_RTVoice = new DSActionsFor_RTVoice();
	...
	// RTVoice Action
	enum DSRTVoiceActionType{None,SayTitle,SayText,SayTitleAndText,Silence}
	class DSActionsFor_RTVoice{
		var action : DSRTVoiceActionType = DSRTVoiceActionType.None;
		var voiceToUse : int = 0;
	}
	...
	enum DS_SendMessageArg{None,SendString,SendInt,SendFloat,SendBoolean,SendGameObject,SendTransform}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//	CACHED STYLE INJECTORS
//	This code helps with injecting styles throughout the system
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// Cached Injectors
class DUICachedInjectors {
	var charIndex : int = 0;							// where was the injector found originally
	var injectorID : int = 0;							// The ID of the Style / Token to use
	var injectorName : String = "";						// The Style name / Token name
	var isToken : boolean = false;						// This injector is a token, not a style.
	var typewriterEventActivated : boolean = false;		// Only run this typewriter event once per injector

	// Debug
	#if UNITY_EDITOR
		var liveID : int = 0;				// The ID of the Style being used live (with offsets)
		var liveOffset : int = 0;			// The live offset between the charIndex and the liveID.
	#endif
}

// This holds all the injectors for the current Screen
class DUICachedInjectorsForScreen{

	// ======================================
	//	THIS PART STORES THE INJECTION DATA
	// ======================================

	// The core injectors are DUICachedInjectors arrays.
	var dialogTitleInjectors : DUICachedInjectors[] = new DUICachedInjectors[0];
	var dialogTextInjectors : DUICachedInjectors[] = new DUICachedInjectors[0];
	var customButton1Injectors : DUICachedInjectors[] = new DUICachedInjectors[0];
	var customButton2Injectors : DUICachedInjectors[] = new DUICachedInjectors[0];

	// Multiple Button Injectors have multiple buttons so they are stored in a new class that allows that.
	var multipleButtonInjectors : DUICachedInjectorsForScreenMultipleButton[] = new DUICachedInjectorsForScreenMultipleButton[0];
	// This one is setup dynamically when evaluating multiple button logic
	var multipleButtonEvaluatedInjectors : DUICachedInjectorsForScreenMultipleButton[] = new DUICachedInjectorsForScreenMultipleButton[0];

	// Icon grid injectors - they have a special class
	var iconGridInjectors : DUICachedInjectorsForScreenIconGrids[] = new DUICachedInjectorsForScreenIconGrids[0];
	// Dynamic version
	var iconGridEvaluatedInjectors : DUICachedInjectorsForScreenIconGrids[] = new DUICachedInjectorsForScreenIconGrids[0];

	// ======================================
	//	THIS IS THE ORIGINAL STRINGS TO USE
	// ======================================
	...
	var customButton2 : String = "";
	var multipleButtons : String[] = [];
	var multipleButtonsEvaluated : String[] = [];	// This is calculated on the fly
	var iconGridTitles : String[] = [];
	var iconGridLabels : String[] = [];
	var iconGridFailedLabels : String[] = [];
}

// Class that allows for an array of injector arrays
class DUICachedInjectorsForScreenMultipleButton {
	var injectors : DUICachedInjectors[] = new DUICachedInjectors[0];
}

// Class that allows for an array of injector arrays
class DUICachedInjectorsForScreenIconGrids {
	var title : DUICachedInjectors[] = new DUICachedInjectors[0];
	var label : DUICachedInjectors[] = new DUICachedInjectors[0];
	var failedLabel : DUICachedInjectors[] = new DUICachedInjectors[0];
}

// This class is returned when running CalculateStyleInjectors()
class ReturnDUICachedInjectorsForScreen{
	var injectors : DUICachedInjectors[] = new DUICachedInjectors[0];
	var theText : String = "";
}
